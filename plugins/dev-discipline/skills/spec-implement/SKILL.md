...
- effort-router の判定(T0〜T4)に従って effort/モデルを調整する(Claude Code の場合。`/effort` `/model` `/fast` のセッション設定の適用はユーザーに促す)。
- 判定が **T3/T4(大・探索)ならプランモード相当での開始**を案内する。AI 側で制御できる範囲(プランモードに入る、調査をサブエージェントに外出しする等)は effort-router の定義どおり自律的に適用してよい。
- 調査の外出しは、**独立して並列化できる広域調査に限る**。数回のツール呼び出しで終わる調査は自分で行い、実装の検証・二重チェックにサブエージェントを使わない(検証は手順4の実際のテスト/リンタ実行で行う)。
- **`token-ops`(effort-router スキル)が未導入の場合**は、規模(触るファイル数)・不確実性・リスクを自分で一言ずつ見積もり、過不足ない進め方を選ぶ。

### 3. 実装前の確認

- 受け入れ条件とスコープ外を一度だけ簡潔に再掲する(受け入れ条件を満たすことがゴール、スコープ外には踏み込まない)。手順1の要約と重複させて二度書かない。
...
**結論**: [1行。例: 全4タスク完了、受け入れ条件5件すべて達成、テスト/リンタ green]
**方針ドキュメント**: [パス]
...
- タスクごとの逐次実況はしない。重要な発見や方針変更があったときだけ短く報告し、まとめは手順5の完了報告で行う。
