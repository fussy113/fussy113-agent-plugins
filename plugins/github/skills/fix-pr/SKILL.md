...
CI 失敗とレビューコメントのすべてに対応方針(修正 or 対応しない理由)が付き、手順9のサマリーが出力されていれば完了とする。手順8(コメント返信・スレッド解決)は外部への書き込みのため、進める前にユーザーの確認を得る。

## 進め方の伝え方

工程が多いスキルなので、実況で埋めずに要点だけ伝えてください。

- 最初のツール呼び出し前に、これから何をするかを1文で述べる。
- 作業中は、重要な発見(CI 失敗の原因が判明した、修正方針を変えた等)があったときだけ短く報告する。手順ごとの逐次実況はしない。
- 完了時は結論から始める。詳細はその後に置く。
...
ここから先は他者に見える外部への書き込み(コメント投稿・スレッド解決)です。修正コミットまでの内容をユーザーに一度提示し、確認を得てから進めてください。
...
**結論**: [1行。例: CI 2件のうち1件を修正(1件はインフラ起因)、レビュー3スレッドに返信・解決]
...
- 指摘内容を正確に理解してから修正に着手する。不明な場合は安易に変更しない
- スタイルや好みの問題で既存コードの規約に従っている場合は、その旨を説明して変更しない
- PR差分のスコープ外の変更(無関係なリファクタリング等)は行わない
- サブエージェントへの委譲は、独立した広域調査(多数のファイルにまたがる原因追跡等)に限る。数回のツール呼び出しで終わる調査は自分で行い、自分の修正の検証・二重チェックには使わない(検証は手順6の実際のテスト/リンタ実行で行う)
